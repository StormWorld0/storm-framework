...
class PostgresManager:
    ...
    def _run_cmd(self, cmd, capture_output=False, check=True):
        ...
        try:
            ...
            smf.printd(f"Command failed with exit code {e.returncode}: {' '.join(cmd)}", level="ERROR")
            if e.stderr:
    ...
    def init_database(self):
        ...
        if config.DB_DATA_DIR.exists() and any(config.DB_DATA_DIR.iterdir()):
            smf.printd("The database directory already exists and is not empty. Skip initdb.", level="WARN")
            return
    ...
    def start(self):
        ...
        self._run_cmd([
            self.pg_ctl, "-D", str(config.DB_DATA_DIR), 
            "-l", str(config.DB_DATA_DIR / "pg.log"), 
            "-o", f"-p {config.DB_PORT}", "start"
        ], check=False) # check=False karena pg_ctl bisa return non-zero jika sudah jalan
    ...
    def status(self):
        ...
        result = self._run_cmd([self.pg_ctl, "-D", str(config.DB_DATA_DIR), "status"], capture_output=True, check=False)
    ...
    def provision_roles_and_db(self):
        ...
        self._run_cmd([
            self.createuser, "-p", config.DB_PORT, "-h", config.DB_HOST, "-s", config.DB_USER
        ], check=False)
        ...
        self._run_cmd([
            self.createdb, "-p", config.DB_PORT, "-h", config.DB_HOST, "-O", config.DB_USER, config.DB_NAME
        ], check=False)
